...
async function main() {
  ...
  // await hre.run('compile');
    ...
    try{
      ...
      const cryptoBeetles = await CryptoBeetles.deploy("XT.COM Crypto Beetles", "XTNFT");

      await cryptoBeetles.deployed();
      console.log(`CryptoBeetles successfully deployed to: ${cryptoBeetles.address}`);

      const newItem = await cryptoBeetles.mint("https://ipfs.io/ipfs/QmSX1ocRr7torptLkc4jKAd64jRrTpxSUAmMk13fP56CuV");
      console.log(`New NFT minted successfully:: ${newItem}`);

      await cryptoBeetles.mint('https://ipfs.io/ipfs/QmbpwTRwSUUCRc7vM4dnZayFwMbPT3RLw7TSdwRSU3ZB2M');
      console.log(`New NFT minted successfully`);

      await cryptoBeetles.mint('https://ipfs.io/ipfs/QmWpiygCB7tnaY6T44f8t3qYdqorwjanwY9N3u4PHPXs86');
      console.log(`New NFT minted successfully`);
